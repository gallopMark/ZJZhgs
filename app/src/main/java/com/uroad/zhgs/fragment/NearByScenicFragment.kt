...
import com.uroad.zhgs.adapteRv.NearByScenic2Adapter
 ...
 */
class NearByScenicFragment : BasePageRefreshRvFragment() {
    ...
    private lateinit var adapter: NearByScenic2Adapter
    ...
    override fun initViewData(view: View) {
        ...
        adapter = NearByScenic2Adapter(context, mDatas)
        ...
        doRequest(WebApiService.MAP_DATA, WebApiService.mapDataByTypeParams(MapDataType.SCENIC.code,
            ...
            override fun onSuccess(data: String?) {
                ...
                if (GsonUtils.isResultOk(data)) {
                    val mdLs = GsonUtils.fromDataToList(data, ScenicMDL::class.java)
                    updateData(mdLs)
                } else {
    ...
    private fun updateData(mdLs: MutableList<ScenicMDL>) {
        mDatas.clear()
        mDatas.addAll(mdLs)
