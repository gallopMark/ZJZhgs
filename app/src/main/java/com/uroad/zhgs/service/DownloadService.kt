...
import com.uroad.zhgs.utils.AppUtils
import java.io.File
...
class DownloadService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent == null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                val channel = NotificationChannel(NOTIFICATION_ID.toString(), DownloadService::class.java.name, NotificationManager.IMPORTANCE_LOW)
    ...
    private val downloadListener = object : FileDownloadSampleListener() {
        ...
        override fun completed(task: BaseDownloadTask) {
            ...
            AppUtils.installApk(this@DownloadService, File(savePath))
            stopSelf()
        }

        override fun paused(task: BaseDownloadTask, soFarBytes: Int, totalBytes: Int) {
            isPause = true
            val pauseText = "下载已暂停，点击继续！"
